...
class VahanScraper:
    ...
    def fetch_form29(self, reg_no):
        try:
            ...
            for i,s_no in enumerate(s_no_for_form_29):
                s_no = int(s_no) - 1
                all_pages_soup,view_state = self.flow.get_all_pages_soup(soup,view_state,self.cookies,s_no)
    ...
    #         soup,view_state = self.common.prepare_context(reg_no)
    #         all_pages_soup = self.flow.get_all_pages_soup_for_transaction_data1(soup,view_state,self.cookies)
    #         s_no_for_form_29,all_tr_for_s_no = self.extract.get_all_transaction_data1(all_pages_soup)
    #         for i,s_no in enumerate(s_no_for_form_29):
    #             s_no = int(s_no) - 1
    #             dynamic_id, trans_id = self.extract.extract_receipt_button(all_tr_for_s_no,int(s_no))
    ...
    #             # Extract form 29
                # data = self.extract.extract_form29(soup)
                # form_29[f"{form_29_count}st form 29"] = data
                # form_29_count += 1
    #             if i < len(s_no_for_form_29) - 1:
    #                 soup, view_state = self.common.prepare_context(reg_no)
    ...
    #         all_pages_soup = self.flow.get_all_pages_soup_for_transaction_data1(soup,view_state,self.cookies)
    #         s_no_for_form_29,all_tr_for_s_no = self.extract.get_all_transaction_data1(all_pages_soup)
    #         for s_no, view_state in s_no_for_form_29:
    #             s_no = int(s_no) - 1
    #             dynamic_id, trans_id = self.extract.extract_receipt_button(all_tr_for_s_no,int(s_no))
